...
vi.mock('../wasm/browser/intmax2_wasm_lib', () => ({
...
const handlers = [
  ...
  http.get('*/intmax2_wasm_lib_bg.wasm', () => {
...
class MockIntMaxClient {
  ...
  static async init({ environment: _environment }: { environment: 'testnet' | 'mainnet' }) {
    // Mock WASM loading
    await fetch('/intmax2_wasm_lib_bg.wasm');
  ...
  async login() {
    ...
    const challengeData = await this.mockVaultHttpClient.post('/challenge', {
      address,
      type: 'login',
    });
...
describe('IntMaxClient Browser Login Tests', () => {
  ...
  describe('login() call sequence verification', () => {
    it('should call methods in correct order', async () => {
      ...
      client['mockVaultHttpClient'].post.mockImplementation((url: string) => {
        ...
        if (url === '/challenge') {
          return Promise.resolve({ message: 'challenge' });
        }
        return Promise.resolve({ hashedSignature: 'hash', nonce: 123 });
      });
